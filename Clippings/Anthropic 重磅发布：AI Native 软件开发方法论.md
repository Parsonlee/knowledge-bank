...
Anthropic 最近发布了一篇博客文章《The AI Native SDLC playbook》，讨论 **AI进入软件开发之后，整个软件开发生命周期（Software Development Life Cycle，简称 SDLC）应该怎么重做。**
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/0hd8MxUumHNue53QKGxCSbIFjzOXcCS9WQHsLQ1RSxdMKTQYZL2ick0ichPYwic0qibnr1XzDiaNlFtMdic66eYJYHBHoXXsX4XJuLbibfpqicibTica8/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=0)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHNkv3nTIGjeUcDpwW44Dj6PDMg5OJtRxQQr7tSd9NM7Iwxr9zsibzbiaoDDvpsKTd4VVGbNI9mI0nMe7oAUJqg6vbAV01uM1ibjLY/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=1)
...
所以，便有了 **AI Native 新的软件开发方法论。**

## ◈AI Native 的核心改动

==这篇博客里Anthropic 提出的方案是，把单向的开发流水线改成一个循环（Loop）。==

![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHNyGx9hNhibXvARylu747QdNTNFQiaVqFynH8lukr9IBM0ddtmJs5JMrZWb3BsQefGj3Wibcw2qWLArYRPGHDgAKG9dOKSUm8Cbicg/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=2)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHMGOliaGibAicEUx9bJGExkt7s8dluw2iaLIGPQXtt5sSwktkFqF2YLSNJe8XAUJwxibzdy5uBfzKbK1icynb9EkbdPDPreKJt0OoXQI/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=3)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHOXLm1ue5jhZE7ibU7uJFNw5ISAKeclDicmGf9CvrJ7xcPudjMVXShWVhNG68vUIHLZ6RaOcdxO6YamIfJxypXzTFGdjPmbCERvA/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=4)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/0hd8MxUumHP0Ph4nuN4pN1lDt0yDE8KpQwOw1WVwf15nbNhqhibT0aCa3d7T16brdceb8CqpdicSiaBd4JzDVvnClMgg4Zo6ZcdKa9icJAYJdMY/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=5)

AI 编程最常翻的车，是你让它改个东西，它一下生成几十个文件，你一看方向从头就错了，返工比自己写还累。

所以这一步 **要求 AI 先进入计划模式** ：读代码库、列出要改哪些文件、怎么验证，但计划被你接受前不许动代码。

开发人员可以反复跟它改这份计划，改到满意，通过的版本写进 `plan.md` 。

**AI 动手之前，还可以给它准备三层规则。它们作用不同，也不需要一次全部配齐。**

**最基础的是** `CLAUDE.md` ，一份放在项目根目录的说明文件。

记录怎样构建和测试、目录分别负责什么、哪些位置不能碰，以及 AI 经常犯哪些错误

![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHMahkYNbU3LuuUJqKaSBndH2cutx3QKMRrESnNbACicLxcOKaOvxJhZvFwoqZkUfjII1JfIn5ZMbKrRqa3xPrV5WpP2vwJL3jMg/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=6)

AI 每次进入项目都会先读取它，相当于开工前先看一遍项目说明和避坑指南。。

**其次是一份** `Skill` ，一条条可复用的工作规范。

它主要是管"某类活该怎么干"。比如"对外接口必须先验证身份"，写成一条 Skill，AI 每次做这类任务都照办。

它比 `CLAUDE.md` 更聚焦，针对的是具体某一类操作。

![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHPsdtk82DUQQTja9fI1uLQW0H37N8RWF5eWgu4LJ40QwD5noeOiarvx4sz7oXswSFSvja2KOtLTq0IDs2RdZ2S9IlGvwFg0NTUo/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=7)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHPN4JsMhZDpYcgEIMxSiaj69s8ziaU87ckFwZibPWtclOBk4zrUA89tflTCJUgf9XEo5Kv2ibEGVvocMg51jELpSg9ZuX4e0xjVxGM/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=8)

随着开发任务越来越复杂，Claude 还可以把部分工作拆给 Subagent。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/0hd8MxUumHN8AflXCw1umazNb5Gl5shd5trvvfxyT7O1IZiau1VrDcjatiaYZlHErP4QandR8BjNJpjQBfuX9w5ewgF4hkk5wBflSGB5sq1s8/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=9)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/0hd8MxUumHMWqe8GeQiczHYqbnSthIIOxZp7UgfIcib4EZw9NdaPhoT2BawNFkm2RCFFnv1A4PWEz4RkN6garDDiafXiadsq6eGLib21Bo7Ng8VE/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=10)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHODLQWAibRXD82Ia4H3agjVIoicapz1n90ic5icX9c262icV8XCSPCvmFwVRM62UtyWEHxlv2CibqgMKYreSgPYwC8Zda477S3ibzVzv8/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=11)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHOExicQcicL77Dypf1KhMqrFs0QB7cL3aMar8e7kHz33PCzMRDNCqelu78jIMtbbttjpK49sFhZoDhgVC34dLnFgTqzLcfXXYpKk/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=12)

监控脚本盯住错误率、延迟这类指标，一旦越界就自动叫 AI 做只读诊断（只查不改），严重时提出修复或触发事先批准好的回滚。

需要注意的是 **事先批准** ，AI 能触发的动作都是提前定好、演练过的。

修完确认指标回到正常，这次的处理结论又被写成一份新的 `intent.md` ，回到第一步。

于是，Loop跑通。

![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/0hd8MxUumHOWAobibMNZnTUCvXh0JiaJu45Oia9vravevxY4EibdQmVjlcbNDv3fZhDlwCQnDlQhK0Q1v1Nz5ll1vEED8B4QXKDlDGV3rOiauT3w/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=13)

## ◈该按什么顺序去落地

上面六个阶段讲的是一个需求从想法到上线的流程顺序，但落地的时候不用按这个顺序改。文章给了一张依赖关系图，告诉你可以按什么顺序落地。

![图片](https://mmbiz.qpic.cn/mmbiz_png/0hd8MxUumHMibv5MTlgKGVwazXDheYTMxDNMObBQvgFJIsoiaTqJaGLx1oSXxalE8ibxPWiabq2qqwt1fnoWnnsI1RLicegce36uoBQwnicEDloaI/640?wx_fmt=png&from=appmsg&watermark=1&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=14)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/5fknb41ib9qEyDKnkjcT4bd38ljNdEGscMzUYibunoJ8KWC3aUv6EUpdes1rbU2Kp7TQXqFwMicLuciaz9q7tiaI3UQ/640?wx_fmt=png&from=appmsg&tp=webp&wxfrom=5&wx_lazy=1#imgIndex=15)
