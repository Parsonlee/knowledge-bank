...
传统 Serving 框架的瓶颈
1. 单进程独占：vLLM 和 TEI 均基于“一个模型独占一张 GPU”的假设构建。若在一张 GPU 上强行开启 4 个独立 Server 进程，无法进行跨进程显存协调与优先级调度。
2. 预分配浪费：vLLM 启动时默认按 --gpu-memory-utilization 0.9 预留显存，多个进程极易引发 OOM（内存溢出）。
