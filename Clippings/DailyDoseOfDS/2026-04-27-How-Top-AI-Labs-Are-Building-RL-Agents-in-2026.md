...
相对评分优于绝对评分：LLM 很难打出校准精准的绝对分，但在同一 Prompt 下对比 4-8 条轨迹并按质量排序非常准确。

直接对接 GRPO：GRPO 本身就只需要组内相对排名。RULER 将系统 Prompt 作为评判标准，直接输出 0~1 的相对梯度得分。

无需编写规则代码：修改系统 Prompt 后，RULER 的评判标准会自动自适应调整，无需重写任何 Reward 函数。
