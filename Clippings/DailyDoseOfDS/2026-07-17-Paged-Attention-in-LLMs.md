...
1. 块级分配（Block-level Allocation）：将 KV Cache 切分为固定大小的物理块（通常为 16 个 Token 一块），块在 GPU 显存中无需连续。
2. 页表映射（Block Table）：每个请求维护一个页表，将逻辑 Token 索引动态映射到离散的物理显存块上。
3. 共享 Prefix 缓存：多个共享相同 System Prompt 的请求可直接复用同一物理显存块，直到输出分叉时才写时复制（Copy-on-Write）。
