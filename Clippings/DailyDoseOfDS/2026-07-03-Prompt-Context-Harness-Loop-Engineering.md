...
Prompt Engineering：关注单次调用的词调与结构（CoT、Few-shot、Output Schema），影响模型内部推理。

Context Engineering：管理单轮可见的全部上下文（检索 Chunk、历史对话、内存），对输入进行优先级排序与裁剪。

Harness Engineering：构建模型周围的代码支撑（工具定义、异常重试、格式解析、Sub-agent 路由与 Verifier 校验）。

Loop Engineering：掌控整个 Task 运行生命周期（设置 Turn/Token 上限、无进展检测、自动化触发与终止信号）。
