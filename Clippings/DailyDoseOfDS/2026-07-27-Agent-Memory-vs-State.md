title: "Agent Memory 与 State 完全是两回事！" date: 2026-07-27 author: "Avi Chawla & Akshay Pachaar" source: "https://mail.google.com/mail/u/0/#inbox/19fa5754b2a0ee28" type: clipping
...
核心要点总结
1. 将 Memory 与 State 明确区分为两个独立的问题。
2. 当发现不应共享时，按 Agent 独立隔离 Memory 作用域。
3. 在每个已完成的任务步骤后写入 Checkpoint（检查点）。
4. 中断运行后，从最新的 Checkpoint 恢复执行。
5. 支持将 Checkpoint 分叉（Fork）出新分支，无需重复之前的执行过程。