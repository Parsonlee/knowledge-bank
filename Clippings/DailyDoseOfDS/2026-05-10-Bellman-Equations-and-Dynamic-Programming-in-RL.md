...
贝尔曼期望方程（Bellman Expectation Equation）：拆解当前奖励与未来折现价值的关系。

贝尔曼最优方程（Bellman Optimality Equation）：求解最优策略。

动态规划方法：迭代策略评估（Iterative Policy Evaluation）、策略改进（Policy Improvement）以及价值迭代（Value Iteration）的原理与代码实现。
