...
Cross-encoders（交叉编码器）：将 Query 与 Document 拼接后共同送入 Transformer，利用 Attention 进行充分交叉交互。语义表达最强，但每次检索需实时前向计算，无法扩展至千万级文档。

Bi-encoders（双编码器）：Query 与 Document 分别独立编码，预先将文档向量化存入 Vector DB，在线计算余弦相似度。极具可扩展性，但丢失了词级交互信息。

ColBERT（Late Interaction 延迟交互）：保留文档每个 Token 的向量表示，在 Query 与 Document Token 之间计算点积矩阵，并取 MaxSim 累加。既支持文档向量离线索引，又保留了精确的词级交互。
