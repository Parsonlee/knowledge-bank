...
BM25 的 3 大核心回答：
1. 词的稀有度（IDF）：提升稀有词权重，忽视通用停用词。
2. 词频饱和度（Term Frequency Saturation）：词出现 10 次与 100 次的边际效益递减，通过参数 k1 抑制高频词过调。
3. 文档长度归一化（Document Length Normalization）：避免长文档因天然包含更多词而作弊得分，通过参数 b 惩罚超长文档。
