...
1. Query Span：捕获用户的原始输入、时间戳与 Session 信息。
2. Embedding Span：记录文本向量化的 Token 消耗与 API 延迟。
3. Retrieval Span：记录向量数据库匹配过程（Exposure 常见的坏 Chunk、低相关度得分、Top-K 参数不当等问题）。
4. Context Span：记录拼接进入 System Prompt 的实际上下文，及时发现 Context 过长问题。
5. Generation Span：记录 LLM 生成回复的输入/输出 Token 数量、延迟与推理过程，用于精确成本核算。
