...
阶段 1（拆解）：Orchestrator 在无工具状态下将复杂 Query 拆解为最多 6 个独立的研究子方向。

阶段 2（分发执行）：派发 3 个隔离的子研究 Agent，各自进行最多 8 轮搜索、阅读与思考，并允许检索企业内部文档（Slack、Confluence 等）。

阶段 3（聚合）：通过确定性代码进行去重、重新编号，生成附带统一引用地图的高质量报告。
