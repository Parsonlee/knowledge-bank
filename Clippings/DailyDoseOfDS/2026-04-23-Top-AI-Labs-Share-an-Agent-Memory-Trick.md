...
Relational（关系/关系型数据库）：追溯层（Provenance），记录事实的来源、存储时间与访问权限。

Vector（向量数据库）：检索层（Retrieval），负责语义相似度搜索。

Graph（图数据库）：推理层（Reasoning），记录事实之间的显式连接与依赖关系（如 Mark ➔ 10年级 ➔ 3月考试 ➔ 图书馆关闭）。
