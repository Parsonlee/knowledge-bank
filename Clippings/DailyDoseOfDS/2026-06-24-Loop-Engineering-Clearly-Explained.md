...
拒绝由模型自我判定“完成”：避免 Agent 未跑测试即汇报完成。引入独立 Verifier 评估器（如测试通过或确定性校验）作为真正的终止信号。

Context Rot 与 Doom Loop 防御：循环时间越长，上下文越容易充满无用 Tool 输出与废弃推理，引发“死亡循环”。必须采用 Compaction（压缩）、Offloading（文件卸载）及独立 Sub-agent 分流。

幂等工具设计与明确错误提示：循环重试需要安全且幂等的写工具，且错误提示必须指明“下一步如何修正”，而非仅抛出异常。
