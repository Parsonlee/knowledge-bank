...
显存与存储剧降：每个用户的 LoRA 适配器权重仅约 20-25 MB。

多租户热插拔：OpenAI / 部署服务只需在 GPU 中常驻一份基座模型，在收到请求时将轻量级 LoRA 矩阵与基座权重实时相加（$\Delta W = B \times A$），即可瞬间切换不同用户的定制模型。

QLoRA 进一步压缩：通过 NF4（NormalFloat4）数据类型量化基座模型，配合双重量化（Double Quantization）与分页优化器（Paged Optimizers），使得单张消费级 GPU 即可完成 70B 模型的微调。
