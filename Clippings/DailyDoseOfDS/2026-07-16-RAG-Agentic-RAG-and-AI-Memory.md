...
1. 传统 RAG (2020-2023)：单次检索 + 生成回复，无决策能力，容易检索出无关上下文。
2. Agentic RAG：由 Agent 自主判断是否需要检索、选择哪个数据源以及验证结果可用性。但依然是只读模式。
3. AI Agent Memory：支持读写交互。Agent 不仅能读取外部知识，还能将用户偏好、历史交互经验写入知识库，实现无需重新训练的持续学习（Continual Learning）。
