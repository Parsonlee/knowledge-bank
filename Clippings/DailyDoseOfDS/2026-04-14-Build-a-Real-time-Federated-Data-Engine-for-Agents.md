...
核心特性：
1. 数据原地保留 (In-place)：彻底消除 ETL 管道与数据重复。
2. 跨源实时 JOIN：可以直接编写 SQL，将 Postgres 表与 MongoDB Collection 以及 REST API 进行实时关联查询。
3. 自然语言转 SQL：支持用自然语言描述需求，MindsDB 自动将其转化为底层高效 SQL 执行。
4. Agent 零延迟：源头数据一旦变动，Agent 即可立即检索到最新结果，消除了 Embedding 向量过期的难题。