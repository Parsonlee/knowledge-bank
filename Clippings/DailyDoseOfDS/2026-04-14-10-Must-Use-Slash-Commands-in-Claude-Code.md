...
动态参数：使用 $ARGUMENTS 接收命令后输入的参数（例如 /dissect src/auth.ts）。

动态 Shell 上下文：使用 !git diff 或 !pytest 语法，Claude 会在处理 Prompt 之前先执行 Shell 命令并插入最新的输出。

YAML Frontmatter 预授权：可设置 allowed-tools 预批准工具权限，避免频繁弹出确认提示。
