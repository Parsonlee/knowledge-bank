...
| 目录 | 用途 |
|------|------|
| `notes/` | 用户手写的原创笔记、指南、独立文章（Agent 不主动修改） |
| `inbox/` | 空的速记收件箱 |
| `assets/` | 附件（图片、PDF） |
...
1. **深度阅读与提炼**：阅读原始资料（如 `Cubox/xxx.md`），若本地正文不足则抓取 URL 全文。提炼 3-7 条核心要点与关键引文。
2. **生成 Source 摘要页**：在 `wiki/sources/` 目录下创建对应的 `.md` 摘要页。
   - YAML Frontmatter 格式严格遵守规范：
     ...
     sources: ["Cubox/xxx.md"] # 100% 精准锚定物理文件路径
     updated: "YYYY-MM-DD"
     ---
     ```
...
5. **更新索引与日志**：
   ...
   - 打开 `wiki/log.md` 追加一笔日志记录：
     `## [YYYY-MM-DD] ingest | Cubox/xxx -> wiki/sources/xxx.md (+ affected pages)`
