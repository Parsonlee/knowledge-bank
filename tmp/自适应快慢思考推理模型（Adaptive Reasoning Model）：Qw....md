...
[zhuanlan.zhihu.com](https://zhuanlan.zhihu.com/p/1910075005586346955)chaofachaofa用代码打点酱油，业余偶尔在 b站/油管做视频～关注他
...
qwen3-tech-report-full-architecture
...
qwen3-tech-report-thinking-mixed-mode
...
<math xmlns="http://www.w3.org/1998/Math/MathML"> (2) R ( x , r ) = R base ( x , r ) − α 1 ⋅ P miss ( x , r ) − α 2 ⋅ P over ( x , r ) − γ ⋅ P fmt ( r ) </math> R(x,r) = R_{\\text{base}}(x,r) - \\alpha_1 \\cdot P_{\\text{miss}}(x,r) - \\alpha_2 \\cdot P_{\\text{over}}(x,r) - \\gamma \\cdot P_{\\text{fmt}}(r) \\tag{2}

<math xmlns="http://www.w3.org/1998/Math/MathML"> x </math>x 是 user query， <math xmlns="http://www.w3.org/1998/Math/MathML"> r </math>r 是 model response， <math xmlns="http://www.w3.org/1998/Math/MathML"> P </math>P 都是二元奖励或惩罚，也就是只能是 0 / 1， <math xmlns="http://www.w3.org/1998/Math/MathML"> P m i s s ( x , r ) </math>P_{miss}(x, r) 表示缺少思考， <math xmlns="http://www.w3.org/1998/Math/MathML"> P o v e r ( x , r ) </math>P_{over}(x, r) 表示不应该思考的时候思考了， <math xmlns="http://www.w3.org/1998/Math/MathML"> P f m t ( x , r ) </math>P_{fmt}(x, r) 是格式奖励。最重要的 <math xmlns="http://www.w3.org/1998/Math/MathML"> R b a s e ( x , r ) </math>R_{base}(x, r) 表示当前回复的质量打分，典型的 Model-based Reward Model。

其他细节并不是很清楚，既然写了是 PPO 算法，那么大概率也是 Follow PPO 优化算法公式，如下：

<math display="block" xmlns="http://www.w3.org/1998/Math/MathML"> L KL+CLIP ( θ ) = E t \[ min ( r t ( θ ) A t , clip ( r t ( θ ) , 1 − ϵ , 1 + ϵ ) A t ) − β KL \[ π θ ( ⋅ \| s t ) ‖ π θ ref ( ⋅ \| s t ) \] \] </math>\\begin{align\*} L\^{\\text{KL+CLIP}}(\\theta) = \\mathbb{E}_{t} \\bigg\[ \& \\min\\left( r_t(\\theta) A_t, \\,\\, \\text{clip}\\big(r_t(\\theta), 1-\\epsilon, 1+\\epsilon\\big) A_t \\right) \\nonumber \\\\ \& - \\beta \\, \\text{KL}\\left\[\\pi_{\\theta}(\\cdot\|s_t) \\\| \\pi_{\\theta_{\\text{ref}}}(\\cdot\|s_t)\\right\] \\bigg\] \\end{align\*} \\\\

其中 <math xmlns="http://www.w3.org/1998/Math/MathML"> r t θ </math>r_t{\\theta} 是重要性采样比率，表示为：
...
![](https://cubox.pro/c/filters:no_upscale()?imageUrl=https%3A%2F%2Fpic1.zhimg.com%2Fv2-2f07e3d5e40d2b231d5ba5a3a83b78ca_1440w.jpg&valid=false)
adacot-adathinking-20250525192603224
...
[DeepSeek-R1](https://link.zhihu.com/?target=https%3A//bruceyuan.com/post/deepseek-r1-paper-reading-notes.html) 之后，真就全员 RL 啊，什么东西都用 RL 来搞一遍\~ ok, `RL is all we need~`
